...
class ModelBase:
    def __init__(self, short_name, hf_id, timm_id, device, num_classes, path = None, pretrained=True):
        ...
        if path == None:
            self.model = timm.create_model(timm_id,pretrained=pretrained,num_classes=num_classes)
        else:
            state_dict= load_file(path)
            self.model = timm.create_model(timm_id,num_classes=num_classes)
